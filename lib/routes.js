...
Router.route('/driver-mobile-login/email/:email/password/:password', function () {
    const emailVal = this.params.email;
    const passwordVal = this.params.password;
    const userId=Meteor.loginWithPassword(emailVal, passwordVal, function (err) {
        if (err !== undefined) {
            console.log("error occurs"+err.getReason);
        } else {
